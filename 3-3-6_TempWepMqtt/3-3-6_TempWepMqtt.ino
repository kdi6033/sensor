...
long lastMsg = 0, lastMsg1 = 0;
int delayTime=4000, delayTime1=4000; // serial 계측시간 mqtt로 메세지 보낸는 시간 (단위 ms)
...
const char* outTopic = "/echo/inTopic";
const char* inTopic = "/echo/outTopic";
...
void serialEvent() {
  ...
  temperature=count;
  humidity=55;
  sprintf (msg, "\{'name':'mhtp','no':%d, 'value': %.2f, 'humi': %.2f\}",WiFi.status(),temperature,humidity);
  //sprintf (msg, "\{\"name\":\"mhtp\",\"no\":1,\"temp\": %d,\"humi\":55\}",count);
...
void serialEvent() {
  if(Serial.available()){
    ...
    if( inputString.length() >= 15) {
      ...
      // 온도와 습도 값을 브로커에 보낸다.
      sprintf (msg, "\{\"name\":\"mhtp\",\"no\":%d, \"value\":%.2f, \"humi\":%.2f\}",no,temperature,humidity);
      client.publish(outTopic, msg);
    }
  }
}
