...
void makeHtml() {
  ...
  s+="속도조절<input type='range' name='variable' min='1' max='255' value='"+(String)valueSet+"' onclick='submit();'>";
  //s+="&emsp;<label for='lightSet0'>조도(1~1024):</label><input type='number' name='variable' min='0' max='1024' value='"+(String)valueSet+"' onclick='submit();'>";
  s+="</form>";
  s+="<hr width='100%' align='left' color='gray'>";
  s+="<br>입력전압(V)  ";
  s+=(String)a0+"   "+(String)a1+"   "+(String)a2+"   "+(String)a3;
