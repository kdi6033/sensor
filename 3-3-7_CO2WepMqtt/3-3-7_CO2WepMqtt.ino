...
const char* outTopic = "/iae01/inTopic";
const char* inTopic = "/iae01/outTopic";
...
void CheckMqtt() {
  ...
  if (now - lastMsg > delayTime) {
    ...
    // 센서 값을 브로커에 보낼 메세지 작성.
      sprintf (msg, "\{\"name\":\"co2\",\"no\":%d,\"value\": %.2f}",(int)no, (float)fCO2);
      Serial.println(msg);
...
void serialEvent() {
  if (stringComplete) {
    ...
    String inTag=inputString.substring(2,8);
    fCO2=inTag.toFloat();
    CheckMqtt();
    // clear the string:
    inputString = "";
