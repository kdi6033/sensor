...
const char* outTopic = "/i2r_lg/inTopic";
const char* inTopic = "/i2r_lg/outTopic";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if( sChipID.equals(sChipidin)) {
    ...
    s="write";
    if( s.equals(act)) {
      ac_temperature = root["temper"];
      ac_flow = root["flow"];
      air_swing = root["swing"];
      ac_air_clean_state = root["clean"];
      ac_heat = root["heat"];
       ...
       */
       /*
        s="{\"chipid\":\""+sChipID+"\","+"\"ip\":\""+WiFi.localIP().toString()+","
          +"\"power\":"+ac_power_on+"\"temper\":"+ac_temperature+"\"heat\":"+ac_heat +"}";
       */
       s="{\"power\":";
       s=s+"}";
